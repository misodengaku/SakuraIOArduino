...
{
    uint8_t *request = (uint8_t *)&fileID;
    uint8_t response[8] = {0x00};
    uint8_t responseLength;
    return executeCommand(CMD_START_FILE_DOWNLOAD, 2, request, &responseLength, response);
}

uint8_t SakuraIO::getFileData(uint8_t *buf, uint8_t bufSize, uint8_t *len)
{
    uint8_t result = 0;
    uint8_t responseLength = bufSize;

    result = executeCommand(CMD_GET_FILE_DATA, 1, &bufSize, &responseLength, buf);
    if (result != CMD_ERROR_NONE)
    {
        return result;
    }
...
uint8_t SakuraIO::getFileMetaData(uint8_t *status, uint32_t *size, uint64_t *timestamp, uint32_t *crc32)
{
    uint8_t response[17] = {0x00};
    uint8_t result = 0;
    uint8_t responseLength = 17;

    result = executeCommand(CMD_GET_FILE_METADATA, 0, NULL, &responseLength, response);
    if (result != CMD_ERROR_NONE)
    {
        return result;
    }
    ...
    *status = *response;
    *size = *((uint32_t *)(response + 1));
    *timestamp = *((uint64_t *)(response + 5));
    *crc32 = *((uint32_t *)(response + 13));

    return CMD_ERROR_NONE;
}

uint8_t SakuraIO::getFileDownloadStatus(uint8_t *status, uint32_t *received_size)
{
    uint8_t response[5] = {0x00};
    uint8_t responseLength;
    uint8_t result;
    result = executeCommand(CMD_GET_FILE_DOWNLOAD_STATUS, 0, NULL, &responseLength, response);
    ...
    *status = *response;
    *received_size = *((uint32_t *)(response + 1));
